...
final class AdvancedOperatorsTest extends TestCase
{
    ...
    public function it_filters_using_advanced_text_operators(): void
    {
        ...
        $results1 = RestProcessor::for(User::class, $request1)
            ->allowedFilters(['name'])
            ->get();
        ...
        $results2 = RestProcessor::for(User::class, $request2)
            ->allowedFilters(['name'])
            ->get();
    ...
    public function it_supports_relation_sorting_via_explicit_subquery(): void
    {
        // given
        $u1 = User::create(['name' => 'Alice', 'email' => 'a@test.com']);
        \Warrior\RestProcessor\Tests\Fixtures\Models\Post::create(['user_id' => $u1->id, 'title' => 'Zebra Post']);

        $u2 = User::create(['name' => 'Bob', 'email' => 'b@test.com']);
        \Warrior\RestProcessor\Tests\Fixtures\Models\Post::create(['user_id' => $u2->id, 'title' => 'Apple Post']);
        ...
        $baseQuery = User::query()->addSelect([
            'latest_post_title' => \Warrior\RestProcessor\Tests\Fixtures\Models\Post::select('title')
        ...
        // when
        $results = RestProcessor::for($baseQuery, $request)
